...
const AddTuition = ({ isOpen, onOpenChange, onClose, onCreated }: Props) => {
  // State
  const [cid, setCID] = useState<string>('');
  const [user, setUser] = useState<User>();
  const [code, setCode] = useState<string>(''); // Class code
  const [program, setProgram] = useState<Program>();
  const [type, setType] = useState<Selection>(new Set([tuitionTypes[0].key]));
  ...
  const validateInputs = () => {
    ...
    if (!code && !program) {
      newErrors.code = 'Program code is required or program not found';
    }
