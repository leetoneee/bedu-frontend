...
import { columnsL as columns } from '@/data/payment.data';
...
const LPTab = () => {
  ...
  useEffect(() => {
    if (error) {
      ...
      const listPayments = data.metadata.payments.filter(
        (payment: Payment) => payment.class !== null
      );
  ...
  const filteredItems = React.useMemo(() => {
    ...
    if (filterClassName) {
      filteredPayments = filteredPayments.filter((course) =>
        course.class.name.toLowerCase().includes(filterClassName.toLowerCase())
      );
    }
    return filteredPayments;
  }, [payments, filterTransactionId, filterUserName, filterClassName]);
