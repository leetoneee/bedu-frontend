...
import { deleteUsersProgram } from '@/services/users-programs.service';
...
export default function DeleteUserProgramModal({
  isOpen,
  onOpenChange,
  onClose,
  programName,
  ...
  // useEffect(() => {
  //   if (userProgramId && userProgramTitle) {
  //     setName(userProgramTitle);
  //     setId(userProgramId);
  //   }
  // }, [userProgramId, userProgramTitle]);
  ...
  const handleDelete = async () => {
    setIsDeleting(true);
    try {
      const result = await deleteUsersProgram(userProgramId);
      if (result) {
        handleClose();
        if (onDeleted) {
