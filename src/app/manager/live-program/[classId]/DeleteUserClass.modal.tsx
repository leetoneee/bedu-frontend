...
import { deleteUserClass } from '@/services/users-classes.service';
...
export default function DeleteUserClassModal({
  isOpen,
  onOpenChange,
  onClose,
  className,
  ...
  // useEffect(() => {
  //   if (userClassId && userClassTitle) {
  //     setName(userClassTitle);
  //     setId(userClassId);
  //   }
  // }, [userClassId, userClassTitle]);
  ...
  const handleDelete = async () => {
    setIsDeleting(true);
    try {
      const result = await deleteUserClass(userClassId);
      if (result) {
        handleClose();
        if (onDeleted) {
            ...
            <ModalBody>
              <div className="space-y-2">
                ...
                <div className="text-sm font-normal">
                  ...
                  <strong>{userClassTitle}</strong> from the program{' '}
                  <strong>{className}</strong> ? This action cannot be undone.
                </div>
              </div>
            </ModalBody>
